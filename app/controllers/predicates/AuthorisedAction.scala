...
class AuthorisedAction @Inject()(
  ...
  private def authorisedAgentForMtdItId(enrolments: Enrolments): Option[AgentDetails] = {
    //  todo possible check for "mtd-it-auth" rule
    for {
      mtdId <- authorisedForMtdItId(enrolments)
      ...
      mtdItId =>
        authorised().retrieve(affinityGroup and allEnrolments) {
          ...
          case Some(AffinityGroup.Agent) ~ enrolments =>
            authorisedAgentForMtdItId(enrolments) match {
