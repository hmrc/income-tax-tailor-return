...
import play.api.mvc.{AnyContent, AnyContentAsEmpty, DefaultActionBuilder, Result}
...
class UserDataControllerSpec
  ...
  private val mockAuth = mock[AuthorisedAction]
  ...
  private val app = new GuiceApplicationBuilder().overrides(bind[UserDataRepository].toInstance(mockRepo),
    bind[AuthorisedAction].toInstance(mockAuth)).build()
...
//
//      val request =
//        FakeRequest(GET, routes.UserDataController.get(taxYear = TaxYear(userData.taxYear)).url)
//          .withHeaders("mtditid" -> userData.mtdItId)
//
//      when(mockAuth.async(any())) thenReturn(Result())
//
